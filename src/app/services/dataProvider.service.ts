...
export class DataProvider {
	...
	public stateStream = new BehaviorSubject(DataProviderState.Init);
	...
	public init(){
		...
		// Set state
		this.stateStream.next(DataProviderState.Init);
		this.errorDetails = '';
		...
		return new Promise((resolve, reject) => {
			...
			this.currentAdapter.init(backendData).then(() => {
				this.stateStream.next(DataProviderState.Ok);
				resolve();
			}).catch((error) => {
				this.stateStream.next(DataProviderState.Error);
